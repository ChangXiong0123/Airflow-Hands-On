...
###############remember to use execute for your own operator###########
    def execute(self, context):
        es = ElasticHook(conn_id = self.elastic_conn_id)
        pg = PostgresHook(postgres_conn_id = self.postgres_conn_id)

        ##now postgres is initialized, we need  to get the connection with postgres
        with closing(pg.get_conn()) as conn:
            with closing(conn.cursor()) as cur: #cur is something to interact with postgres
                cur.itersize = 1000
